...
final class HasManyRelation extends Relation
{
    ...
    public function load(Db $db, array $models): void
    {
        ...
        $grouped = $this->groupBy($related, $foreignKey);
        $this->fill($models, $grouped, $localKey);
    }
    ...
    private function fill(array $models, array $related, ColumnMetadata $localKey): void
    {
        foreach ($models as $model) {
            ...
            $this->relation->setValue($model, $value === null ? [] : ($related[(string) $value] ?? []));
        }
    }
     ...
     */
    private function groupBy(array $models, ColumnMetadata $key): array
    {
        $grouped = [];

        foreach ($models as $model) {
            $grouped[(string) $key->getValue($model)][] = $model;
        }

...
final class SchemaCompilerTest extends TestCase
{
    ...
    public function testPostgresCompilerCreatesTableAndIndexesInSeparateBatches(): void
    {
        $schema = new Schema();
        $schema->create("users", function ($table): void {
            $table->id();
        ...
        $plan = (new PostgresSchemaCompiler())->compile($schema);

        $this->assertCount(2, $plan->batches());
        $this->assertSame([
            'CREATE TABLE "users" ("id" INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, "email" VARCHAR(120) NOT NULL, "bio" TEXT)',
            'CREATE UNIQUE INDEX "users_email_unique" ON "users" ("email")',
        ], $plan->sql());
    }
    ...
    public function testPostgresCompilerMapsExpandedColumnTypes(): void
    {
        $schema = new Schema();
        $schema->create("typed", function ($table): void {
            $table->bigInteger("views");
        ...
        $this->assertSame([
            'CREATE TABLE "typed" ("views" BIGINT NOT NULL, "rating" DOUBLE PRECISION NOT NULL, "price" DECIMAL(12, 4) NOT NULL, "published_on" DATE NOT NULL, "metadata" JSONB NOT NULL, "payload" BYTEA NOT NULL, "public_id" UUID NOT NULL, "active" BOOLEAN NOT NULL, "created_at" TIMESTAMP NOT NULL)',
        ], $plan->sql());
    }
